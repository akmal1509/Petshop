...
class Grooming_m extends CI_Model
{
	...
	public function addItem($id, $qty, $subtotal, $harga, $jenis, $pegawai)
	{
		...
		if ($jenis == "Produk") {
			...
			$this->db->insert('detil_penjualan', $data);
			$detailPenjualan = $this->db->get_where('detil_penjualan', ['kode_detil_jual' => $detiljual])->row();
			$komisi = [
				'id_karyawan' => $pegawai,
				'id_detil_jual' => $detailPenjualan->id_detil_jual,
				'komisi' => 5000,
				'keterangan' => 'Komisi Servis',
				'tanggal' => date('Y-m-d')
			];
