<?php
$pdf = new FPDF('P', 'mm', 'A4');
$pdf->setMargins(6, 10, 20);
// $pdf->SetAutoPageBreak(true,30);
$pdf->AddPage();
$pdf->SetFont('Times', 'B', 18);
$profil = $this->db->get('profil_perusahaan')->row_array();
$pdf->Image('./assets/img/profil/' . $profil['logo_toko'], 6, 4, 13, 13);
$pdf->Cell(25);
$pdf->Cell(0, -6, '', 0, 1);
$pdf->SetFont('helvetica', '', 12);
$pdf->Cell(85, 6, $profil['nama_toko'], 0, 1, 'C');
$pdf->SetFont('helvetica', '', 9);
$pdf->Cell(85, 2, $profil['alamat_toko'], 0, 1, 'C');
$pdf->SetFont('helvetica', '', 8);
$pdf->Cell(85, 6, 'Telp : ' . $profil['telp_toko'] . ' / Fax : ' . $profil['fax_toko'], 0, 1, 'C');


$max_id = "SELECT MAX(id_jual) AS id_jual FROM penjualan";
$id = implode($this->db->query($max_id)->row_array());

if ($id_resi == null) {
	$sql_general = "SELECT a.deskripsi, a.invoice, a.bayar, a.kembali, a.ppn, a.tgl, b.username, b.id_user, a.method FROM penjualan a, user b WHERE a.id_user = b.id_user AND a.id_jual = '$id'";
	$sql_detail = "SELECT b.nama_barang, a.qty_jual, a.diskon, a.subtotal FROM detil_penjualan a,  barang b
    WHERE a.id_barang = b.id_barang AND a.id_jual = '$id' AND a.is_retur = 0";
	$sql_total = "SELECT SUM(subtotal) AS total, SUM(diskon) AS diskon FROM detil_penjualan WHERE id_jual = '$id'";
	$sql_servis = "SELECT a.id_detil_jual, b.kode, b.nama_servis, a.harga_item, a.qty_jual, a.subtotal 
    FROM detil_penjualan a, servis b WHERE b.id_servis = a.id_servis AND a.id_jual= '$id'";
} else {
	$sql_general = "SELECT a.deskripsi, a.invoice, a.bayar, a.kembali, a.ppn, a.tgl, b.username, b.id_user, a.method FROM penjualan a, user b WHERE a.id_user = b.id_user AND a.id_jual = '$id_resi'";
	$sql_detail = "SELECT b.nama_barang, a.qty_jual, a.diskon, a.subtotal FROM detil_penjualan a,  barang b
    WHERE a.id_barang = b.id_barang AND a.id_jual = '$id_resi' AND a.is_retur = 0";
	$sql_total = "SELECT SUM(subtotal) AS total, SUM(diskon) AS diskon FROM detil_penjualan WHERE id_jual = '$id_resi'";
	$sql_servis = "SELECT a.id_detil_jual, b.kode, b.nama_servis, a.harga_item, a.qty_jual, a.subtotal 
    FROM detil_penjualan a, servis b WHERE b.id_servis = a.id_servis AND a.id_jual= '$id_resi'";
}
$general = $this->db->query($sql_general)->row_array();
$detail = $this->db->query($sql_detail)->result_array();
$total = $this->db->query($sql_total)->row_array();
$servis = $this->db->query($sql_servis)->result_array();

$pdf->SetFont('helvetica', '', 8);
$pdf->Cell(70, 6, '-----------------------------------------------------------------------------', 0, 1, 'C');
$pdf->SetFont('helvetica', '', 8);
$pdf->Cell(56, 5, 'NO : ' . $general['invoice'], 0, 0, 'L');
$pdf->Cell(32, 5, $general['method'], 0, 1, 'L');
$pdf->Cell(32, 2, $general['tgl'], 0, 0, 'L');
$pdf->Cell(35, 2, $general['username'] . $general['id_user'], 0, 1, 'R');
$pdf->Cell(70, 6, '-----------------------------------------------------------------------------', 0, 1, 'C');

if ($general['deskripsi'] == NULL) {
} else {
	$pdf->SetFont('helvetica', '', 8);
	$pdf->Cell(56, 5, 'Deskripsi : ', 0, 1, 'L');
	$pdf->Cell(56, 2, $general['deskripsi'], 0, 1, 'L');
	$pdf->Cell(70, 6, '-----------------------------------------------------------------------------', 0, 1, 'C');
}

$pdf->SetFont('helvetica', '', 8);
if ($this->db->query($sql_detail)->num_rows() > 0) {

	$pdf->Cell(32, 5, 'PRODUK :', 0, 1, 'L');
}

foreach ($detail as $d) {
	$pdf->Cell(49, 2, $d['nama_barang'], 0, 0, 'L');
	$pdf->Cell(4, 2, 'x' . $d['qty_jual'], 0, 0, 'L');
	$pdf->Cell(20, 2, $d['subtotal'], 0, 1, 'R');
	$pdf->Cell(60, 5, 'Diskon:', 0, 0, 'R');
	$pdf->Cell(4, 5, $d['diskon'], 0, 1, 'L');
}
$pdf->SetFont('helvetica', '', 8);
if ($this->db->query($sql_servis)->num_rows() > 0) {

	$pdf->Cell(32, 5, 'LAYANAN SERVICE :', 0, 1, 'L');
}
foreach ($servis as $val) {
	$pdf->Cell(49, 3, $val['nama_servis'], 0, 0, 'L');
	$pdf->Cell(4, 3, 'x' . $val['qty_jual'], 0, 0, 'L');
	$pdf->Cell(20, 3, $val['subtotal'], 0, 1, 'R');
}

$pdf->Cell(73, 6, '----------------------------------------', 0, 1, 'R');
$pdf->Cell(54, 3, 'GRAND TOTAL', 0, 0, 'R');
$pdf->Cell(3, 3, ':', 0, 0, 'R');
$pdf->Cell(20, 3, 'Rp. ' . number_format($total['total'], '0', '.', '.'), 0, 1, 'L');
$pdf->Cell(54, 5, 'PPN', 0, 0, 'R');
$pdf->Cell(3, 5, ':', 0, 0, 'R');
$pdf->Cell(20, 5, 'Rp. ' . number_format($general['ppn'], '0', '.', '.'), 0, 1, 'L');
$pdf->Cell(54, 5, 'DISKON', 0, 0, 'R');
$pdf->Cell(3, 5, ':', 0, 0, 'R');
$pdf->Cell(20, 5, 'Rp. ' . number_format($total['diskon'], '0', '.', '.'), 0, 1, 'L');
$pdf->Cell(54, 5, 'TOTAL', 0, 0, 'R');
$pdf->Cell(3, 5, ':', 0, 0, 'R');
$pdf->Cell(20, 5, 'Rp. ' . number_format($total['total'] + $general['ppn'], '0', '.', '.'), 0, 1, 'L');
$pdf->Cell(54, 5, 'TUNAI', 0, 0, 'R');
$pdf->Cell(3, 5, ':', 0, 0, 'R');
$pdf->Cell(20, 5, 'Rp. ' . number_format($general['bayar'], '0', '.', '.'), 0, 1, 'L');
$pdf->Cell(54, 5, 'KEMBALI', 0, 0, 'R');
$pdf->Cell(3, 5, ':', 0, 0, 'R');
$pdf->Cell(20, 5, 'Rp. ' . number_format($general['kembali'], '0', '.', '.'), 0, 1, 'L');

$pdf->Cell(73, 8, '', 0, 1, 'R');
$pdf->Cell(73, 4, '', 0, 1, 'C');
$pdf->Cell(73, 4, '=== TERIMA KASIH SUDAH BERBELANJA ===', 0, 1, 'C');
$pdf->Cell(73, 4, ' BARANG YANG SUDAH DIBELI TIDAK BOLEH DIKEMBALIKAN', 0, 1, 'C');

$pdf->Output($general['invoice'] . '.pdf', 'I');
