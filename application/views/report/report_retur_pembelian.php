...
$pdf->Cell(0, 5, 'LAPORAN RETUR PEMBELIAN', 0, 1, 'C');
$pdf->SetFont('Times', 'B', 11);

$pdf->Cell(0, 7, 'Periode :' . $awal . ' s/d ' . $akhir, 0, 1, 'C');
$sql = "SELECT a.id_retur_pembelian, a.nomor, a.created_at, b.kode_beli, c.nama_lengkap
        FROM retur_pembelian a, pembelian b, user c
        WHERE a.id_pembelian = b.id_beli AND a.id_user = c.id_user
        AND SUBSTRING(a.created_at, 1, 10) BETWEEN '$awal' AND '$akhir' 
        ORDER BY a.created_at DESC";

$sqldetil = "SELECT a.id_retur_pembelian, b.barcode, b.nama_barang, a.harga, a.jumlah, a.mutasi, a.subtotal
...
foreach ($beli as $j) {
    ...
    $pdf->Cell(25, 6, 'KODE', 0, 0, 'L');
    $pdf->Cell(3, 6, ': ', 0, 0, 'R');
    $pdf->Cell(20, 6, $j['kode_beli'], 0, 0, 'L');
    $pdf->Cell(30, 6, '', 0, 1);
    $i = 1;
    $a = 1;
    ...
    foreach ($detil as $d) {
        if ($j['id_retur_pembelian'] == $d['id_retur_pembelian']) {
            $pdf->SetFont('Times', '', 9);
            $pdf->Cell(7, 6, $i++, 1, 0);
