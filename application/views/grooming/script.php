<script>
			...
			"aoColumns": [{
				...
				},
				{
					"mDataProp": function(data, type, val) {
						let render = `<img src="<?php echo base_url('assets/img/produk/') ?>${data.gambar}" alt="" style="max-width:90px">`;
	...
	function pilihservis(e) {
		$.ajax({
			...
			success: function(data) {
				...
				$('#kode_servis').val(e).trigger('change')
			}
		})
	}
	...
	$('#customer').on('change', function() {
		if ($(this).val() != 1) {
			$.ajax({
				url: base_url + "grooming/contact_info/" + $(this).val(),
				type: "get",
				success: function(data) {
					var obj = JSON.parse(data);
					$('#telp').val(obj.telp);
	...
	$('#kode_servis').on('change', function() {
		let idServis = $(this).val();
		$.ajax({
			url: base_url + "servis/detail/" + idServis,
			type: "post",
			success: function(data) {
				var obj = JSON.parse(data);
				$('#harga').val(obj.harga);
