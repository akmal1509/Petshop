...
class Dpenjualan extends CI_Controller
{
  ...
  public function index()
  {
    $penjualan = $this->db->query("
        SELECT b.id_jual, b.kode_jual, b.invoice, d.nama_lengkap, c.nama_cs, SUM(a.diskon) diskon, SUM(a.subtotal) as total, b.tgl, SUM(a.qty_jual) AS qty, b.method 
        FROM detil_penjualan a, penjualan b, customer c, user d 
        WHERE b.id_jual = a.id_jual AND c.id_cs = b.id_cs AND d.id_user = b.id_user AND b.is_active= 1 AND b.type = 'sales' GROUP BY a.id_jual
    ")->result();
    $retur = $this->db->query("
        SELECT a.id_penjualan, SUM(b.subtotal) as nilai
        FROM retur_penjualan a, detil_retur_penjualan b
        WHERE a.id_retur_penjualan = b.id_retur_penjualan GROUP BY b.id_retur_penjualan
    ")->result();
    $data = array(
  ...
  {
    $sql = "SELECT a.kode_detil_jual, c.barcode, c.nama_barang, a.harga_item, a.qty_jual, a.diskon, a.subtotal FROM detil_penjualan a, penjualan b, barang c WHERE b.id_jual = a.id_jual AND c.id_barang = a.id_barang AND  a.id_jual = '$id' AND a.is_retur = 0";
